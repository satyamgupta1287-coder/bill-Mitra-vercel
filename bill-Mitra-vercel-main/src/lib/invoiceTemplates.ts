...
// Builds "address, city" without repeating the city/state if it's already part of the address text
const fullAddress = (company: any) => {
  const address = String(company?.address ?? '').trim();
  const city = String(company?.city ?? '').trim();
  if (!city) return esc(address);
  if (address.toLowerCase().includes(city.toLowerCase())) return esc(address);
  return esc(address) + (address ? ', ' : '') + esc(city);
};
...
<div class="border">
  <div class="header">
    <div style="display:flex;align-items:center;gap:12px">
      ...
      <div style="font-size:7.5pt;color:#444">${fullAddress(company)}</div>
...
<div class="accent header">
  <div style="display:flex;align-items:center;gap:14px">
    ...
    <div style="font-size:8pt;opacity:0.85">${fullAddress(company)} | Ph: ${esc(company?.phone)}</div></div>
  </div>
...
.wrap { border: 1px solid #000; padding: 0; display: flex; flex-direction: column; min-height: 277mm; }
...
.dual-sign div{text-align:center;font-size:8pt}.dual-sign span{border-top:1px solid #333;padding-top:4px;display:inline-block}
  ...
  <div class="hdr"><div style="display:flex;align-items:center;gap:12px">${logoHtml(company, opts.showLogo)}<div><h1>${esc(company?.companyName)}</h1><div style="font-size:8pt;color:#555">${fullAddress(company)} | GSTIN: ${esc(company?.gstin)}</div></div></div><div class="inv-label">${esc(invoice.type || 'TAX INVOICE')}</div></div>
  <div class="grid">
...
<div style="font-size:7pt">${fullAddress(company)}</div>
